...
from sqlalchemy import Column, Date, Text, Boolean, Float, Integer, SmallInteger, ARRAY
from sqlalchemy import Enum as SQLEnum
from sqlalchemy.dialects.postgresql import UUID

from .base import Base
from .enums import PlatformType, CategoryType
...
class SrvDailyReviewList(Base):
    ...
    date = Column(Date, nullable=False, comment='리뷰 작성일 (파티션 키)')

    # -- 서비스 컨텍스트
    service_id = Column(UUID(as_uuid=True), comment='서비스 ID')
    app_id = Column(UUID(as_uuid=True), comment='앱 ID')
    ...
    # -- 리뷰 원본
    platform_review_id = Column(Text, comment='플랫폼 원본 리뷰 ID')
    ...
    action_confidence_score = Column(Float, comment='조치 필요 신뢰도')

    # -- 애스펙트 집계 (배열 denormalize)
    keywords = Column(ARRAY(Text), comment='추출된 키워드 배열')
    categories = Column(
        ARRAY(SQLEnum(CategoryType, name='category_type', create_type=False)),
        comment='카테고리 배열 (오방성)',
    )
    avg_sentiment_score = Column(Float, comment='평균 감성 점수')
    aspect_cnt = Column(Integer, default=0, comment='추출된 애스펙트 수')

    # -- 배정 결과
    assigned_dept = Column(ARRAY(Text), comment='배정 부서 목록')
