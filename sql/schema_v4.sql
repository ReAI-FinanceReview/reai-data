-- ========================================
-- DATABASE INITIALIZATION - Schema v4
-- ========================================
-- File: schema_v4.sql
-- Date: 2026-03-05
-- Description: Financial App Review ETL Pipeline Schema (v4)
-- Architecture: Hybrid Storage (PostgreSQL + MinIO/NAS Parquet)
--               Bronze → Silver → Gold → Data Mart layers
--
-- Changes from v3:
--   - ingestion_batch_status_type ENUM 추가
--   - ingestion_batch 테이블 추가 (Batch DLQ 관리)
--   - review_master_index.retry_count 의미 축소: downstream 처리 실패 전용
--   - review_master_index.error_message 코멘트 수정: ingestion 오류 언급 제거
--   - idx_review_master_index_failed, idx_review_master_index_retry 인덱스 제거
--   - ingestion_batch 전용 인덱스 추가
--
-- Total Tables: 19
--   - Existing from v3: 18 tables
--   - New in v4: 1 table (ingestion_batch)
...
CREATE TYPE ingestion_batch_status_type AS ENUM (
    'PENDING', 'LOADED', 'FAILED', 'RETRYING', 'DEAD_LETTER'
);
...
COMMENT ON TYPE ingestion_batch_status_type IS 'Parquet 배치 적재 상태 (DLQ 관리)';
...
COMMENT ON COLUMN review_master_index.parquet_written_at IS 'Parquet 쓰기 성공 시각';
COMMENT ON COLUMN review_master_index.storage_path IS 'MinIO object storage path (s3://bucket/path/file.parquet)';
COMMENT ON COLUMN review_master_index.error_message IS '실패 사유 (downstream 처리 에러 메시지)';
COMMENT ON COLUMN review_master_index.retry_count IS '재시도 횟수 (downstream 처리 실패 전용)';
...
-- ----------------------------------------
-- Ingestion Batch (Parquet Batch DLQ)
-- ----------------------------------------
...
(
  ...
  content_hash    TEXT,
  status          ingestion_batch_status_type NOT NULL DEFAULT 'PENDING',
...
COMMENT ON TABLE ingestion_batch IS 'Parquet 배치 적재 관리 (DLQ) - crawl 단계에서 생성, load 단계에서 소비';
COMMENT ON COLUMN ingestion_batch.batch_id IS '배치 고유 ID (UUID)';
COMMENT ON COLUMN ingestion_batch.source_type IS '플랫폼 타입 (APPSTORE / PLAYSTORE)';
COMMENT ON COLUMN ingestion_batch.platform_app_id IS '플랫폼 앱 ID (store ID / package name)';
COMMENT ON COLUMN ingestion_batch.app_name IS '앱 이름 (optional, from crawl)';
COMMENT ON COLUMN ingestion_batch.storage_path IS 'Parquet 파일 경로 (UNIQUE)';
COMMENT ON COLUMN ingestion_batch.file_format IS '파일 포맷 (기본: parquet)';
COMMENT ON COLUMN ingestion_batch.record_count IS '배치 내 리뷰 레코드 수';
COMMENT ON COLUMN ingestion_batch.content_hash IS '파일 내용 해시 (중복 검사용)';
COMMENT ON COLUMN ingestion_batch.status IS 'PENDING (적재 대기) / LOADED (적재 완료) / FAILED (적재 실패) / RETRYING (재시도 중) / DEAD_LETTER (최대 재시도 초과)';
COMMENT ON COLUMN ingestion_batch.retry_count IS '적재 재시도 횟수';
COMMENT ON COLUMN ingestion_batch.max_retries IS '최대 재시도 횟수 (기본: 3)';
COMMENT ON COLUMN ingestion_batch.error_message IS '적재 실패 사유';
COMMENT ON COLUMN ingestion_batch.created_at IS '배치 생성 시각 (Parquet 쓰기 완료 시각)';
COMMENT ON COLUMN ingestion_batch.updated_at IS '배치 상태 수정 시각';
COMMENT ON COLUMN ingestion_batch.loaded_at IS 'DB 적재 완료 시각';
...
-- Ingestion batch indexes
CREATE INDEX idx_ingestion_batch_status ON ingestion_batch(status);
CREATE INDEX idx_ingestion_batch_pending ON ingestion_batch(status, retry_count)
    WHERE status IN ('PENDING', 'FAILED');
CREATE INDEX idx_ingestion_batch_source_app ON ingestion_batch(source_type, platform_app_id);
...
-- - review_master_index.retry_count: Meaning narrowed to downstream processing failures only
-- - Removed DLQ indexes from review_master_index (moved to ingestion_batch)
