...
PYTHONPATH=. uv run alembic revision --autogenerate -m "describe schema change"
PYTHONPATH=. uv run alembic upgrade head
```

Review autogenerated revisions before committing them. PostgreSQL-specific details such as enum changes, pgvector indexes, partial indexes, partitioned tables, and data backfills must be checked manually because autogenerate does not fully understand every project-level schema decision.

Use this command when a model change should not create a migration:

```bash
PYTHONPATH=. uv run alembic check
```

If it reports new upgrade operations, either create and review a revision or revert the unintended model/schema drift.
